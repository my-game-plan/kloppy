            ...
            for idx, raw_event in enumerate(raw_events):
                ...
                if raw_event.type_id == EVENT_TYPE_START_PERIOD:
                    ...
                    if raw_event.type_id == EVENT_TYPE_PASS:
                        ...
                        # Goals keep the F24 event clock, like every other event.
                        # Qualifier 374 carries a second, independent wall-clock
                        # time for goals only; using it moved goals off the clock
                        # their neighbours are on, which corrupts any feature
                        # derived from the gap between consecutive actions. It
                        # stays available to consumers on `event.raw_event`.
